...
import { DeleteCommentsService, GetCommentsByIDService, GetCommentsService, CreateCommentService } from "../services";
...
export const CreateCommentController = async ( req: Request, res: Response ): Promise<Response> => {
    ...
    const comment: ICommentResponse = await CreateCommentService(data, idUser, idCar);
...
export const GetCommentsByIDController = async ( req: Request, res: Response): Promise<Response> => {
    const id: string = req.params.id;

    await GetCommentsByIDService(id);
